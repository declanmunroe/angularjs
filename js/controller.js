angular.module('RouteControllers', [])
    ...
    .controller('RegisterController', function($scope, UserAPIService, store) {
        ...
        var URL = "https://morning-castle-91468.herokuapp.com/"; //must be the link only and no /docs on the end etc
        
        
        
        $scope.login = function()
        {
            UserAPIService.callAPI(URL + "/accounts/api-token-auth/", $scope.data).then(function(results) //.then means that it is a promice that it will complete the action
            {
                $scope.token = results.data.token;
                store.set("username", $scope.registrationUser.username); //can just use localstoreage.serItem instead of using the library of store
        ...
        $scope.submitForm = function() {
            ...
            UserAPIService.callAPI(URL + "accounts/register/", $scope.registrationUser).then(function(results) {
                ...
                alert("Yay! You have succesfully registered");
                $scope.login();
